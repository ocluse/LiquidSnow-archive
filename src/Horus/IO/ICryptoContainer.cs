...
namespace Thismaker.Horus.IO
{
    ...
    public interface ICryptoContainer
    {
        ...
        Task AddAsync(string name, Stream data, bool overwrite = false, IProgress<float> progress = null, CancellationToken cancellationToken = default);

        /// <summary>
        /// Adds an object to the container, serializing it using the <see cref="HorusIOSettings.Serializer"/>
        /// </summary>
        ...
        ///<inheritdoc cref="AddAsync(string, Stream, bool, IProgress{float}, CancellationToken)"/>
        Task AddBytesAsync(string name, byte[] data, bool overwrite = false, IProgress<float> progress = null, CancellationToken cancellationToken = default);
        
        /// <inheritdoc cref="AddAsync(string, Stream, bool, IProgress{float}, CancellationToken)"/>
        Task AddTextAsync(string name, string data, bool overwrite = false, IProgress<float> progress = null, CancellationToken cancellationToken = default);
        ...
        /// <summary>
        /// Closes the container and disposes the underlying stream
        /// </summary>
        void Dispose();
        ...
        /// <returns></returns>
        Task ExtractContainerAsync(string outputDirecotry, IProgress<float> progress = null, CancellationToken cancellationToken = default);
        ...
        /// <param name="progress">The subscriber notifed of the operation progress</param>
        /// <param name="cancellationToken">The token to monitor for cancellation requests</param>
        Task GetAsync(string name, Stream output, IProgress<float> progress = null, CancellationToken cancellationToken = default);
        
        /// <summary>
        /// Gets, decrypts and deserializes an item from the container using the <see cref="HorusIOSettings.Serializer"/>
        /// </summary>
        /// <param name="name">The name of the item to get</param>
        /// <returns>The resultant object from the operation</returns>
        Task<T> GetAsync<T>(string name);
        
        ///<inheritdoc cref="GetAsync(string, Stream, IProgress{float}, CancellationToken)"/>
        ///<returns>The data in bytes</returns>
        Task<byte[]> GetBytesAsync(string name, IProgress<float> progress = null, CancellationToken cancellationToken = default);
        
        ///<inheritdoc cref="GetTextAsync(string, IProgress{float}, CancellationToken)"/>
        ///<returns>The data as a UTF8 string</returns>
        Task<string> GetTextAsync(string name, IProgress<float> progress = null, CancellationToken cancellationToken = default);
    }
}