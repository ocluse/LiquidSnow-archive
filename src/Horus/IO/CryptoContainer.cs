    ...
    public class CryptoContainer : IDisposable
    {
        ...
        public async Task AddAsync(string filename, Stream input, bool overwrite = false, IProgress<float> progress = null, CancellationToken cancellationToken = default)
        {
            ...
            PackagePart part = _package.PartExists(uri)
                ? overwrite ? _package.GetPart(uri) : throw new UnauthorizedAccessException("Part already exists")
        ...
        #region Object IO
        /// <summary>
        /// Adds an <see cref="object"/> to the container by means of JSON serialization.
        /// Therefore the object, and the data you wish to persist, must be JSON serializable
        /// </summary>
        ...
        public async Task AddAsync<T>(string filename, T o, bool overwrite = false)
        {
            Uri uri = PackUriHelper.CreatePartUri(new Uri(filename, UriKind.Relative));
        ...
        /// <summary>
        /// Can be used to quickly obtain an CLR object that was saved into the container.
        /// </summary>
        ...
        {
            byte[] bytes = await GetBytesAsync(filename, progress, cancellationToken).ConfigureAwait(false);
